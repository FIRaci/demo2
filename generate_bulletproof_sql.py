...
# Note: ProjectStatus enum ONLY accepts: ACTIVE, COMPLETED, ON_HOLD
PROJECTS_DATA = [
    ...
    ("PRJ004", "Hạ tầng Cloud Migration", "Chuyển đổi hệ thống on-premise sang AWS/GCP, tối ưu chi phí và bảo mật.", "ACTIVE"),
    ...
    ("PRJ008", "UI Kit & Design System", "Xây dựng Design System và component library dùng chung cho các sản phẩm.", "ACTIVE"),
    ...
    ("PRJ016", "Tối ưu Quy trình Kế toán", "Số hóa quy trình kế toán, tích hợp phần mềm kế toán với HRM.", "COMPLETED"),
    ...
    ("PRJ019", "Hệ thống Quản lý Kho WMS", "Triển khai Warehouse Management System tích hợp với ERP.", "COMPLETED"),
    ...
    ("PRJ025", "Chiến lược Chuyển đổi Số", "Xây dựng roadmap chuyển đổi số toàn công ty 2026-2028.", "COMPLETED"),
...
def generate_bulletproof_sql():
    lines = []
    lines.append("-- ==========================================================================")
    lines.append("-- DBeaver FULL Seed Data for HRM (Oracle Database)")
    lines.append("-- Hướng dẫn: Mở file này trong DBeaver và nhấn Alt + X để thực thi toàn bộ script.")
    lines.append("-- Đã sửa lỗi ORA-02290: Mọi ProjectStatus đều thuộc (ACTIVE, COMPLETED, ON_HOLD).")
    lines.append("-- Đã dùng Subquery (SELECT id FROM projects WHERE project_code = ...) 100% không lỗi FK!")
    lines.append("-- ==========================================================================")
    lines.append("")
    lines.append("-- 1. Dọn dẹp dữ liệu cũ")
    ...
    # Admin
    lines.append("-- 2. Insert Nhân viên (ADMIN001 + EMP001 -> EMP500)")
    ...
    task_count = 1
    for p_code, p_name, _, _ in PROJECTS_DATA:
        # 5 tasks per project
        for t_idx in range(5):
            ...
            # SUBQUERY TO GET PROJECT ID VIA project_code! NO HARDCODED ID!
            lines.append(f"INSERT INTO tasks (task_code, title, description, deadline, status, project_id, assignee_id) VALUES ('{t_code}', '{title}', '{desc}', TO_DATE('{deadline}', 'YYYY-MM-DD'), '{status}', (SELECT id FROM projects WHERE project_code = '{p_code}'), '{assignee}');")
            task_count += 1

    lines.append("\nCOMMIT;")

    content = "\n".join(lines)
    with open("oracle_seed_full.sql", "w", encoding="utf-8") as f:
        f.write(content)
    with open("oracle_seed_data.sql", "w", encoding="utf-8") as f:
        f.write(content)
    print("Successfully regenerated bulletproof oracle_seed_full.sql & oracle_seed_data.sql!")

if __name__ == "__main__":
    generate_bulletproof_sql()
